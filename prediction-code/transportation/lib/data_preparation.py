...
def read_and_generate_dataset(
    ...
    if not merge:
        ...
        training_set_adj = [
            np.concatenate(i, axis=0) for i in zip(*all_adj[:split_line1])
        ]

    else:
        print("Merge training set and validation set!")
        training_set = [
            np.concatenate(i, axis=0) for i in zip(*all_samples[:split_line2])
        ]
        training_set_adj = [
            np.concatenate(i, axis=0) for i in zip(*all_adj[:split_line2])
        ]
    ...
    validation_set_adj = [
        np.concatenate(i, axis=0) for i in zip(*all_adj[split_line1:split_line2])
    ]

    testing_set = [np.concatenate(i, axis=0) for i in zip(*all_samples[split_line2:])]
    testing_set_adj = [np.concatenate(i, axis=0) for i in zip(*all_adj[split_line2:])]
    ...
    (week_stats, train_week_norm, val_week_norm, test_week_norm) = normalization(
        train_week, val_week, test_week
    )

    (day_stats, train_day_norm, val_day_norm, test_day_norm) = normalization(
    ...
    all_data = {
        ...
        "stats": {"week": week_stats, "day": day_stats, "recent": recent_stats},
    }
